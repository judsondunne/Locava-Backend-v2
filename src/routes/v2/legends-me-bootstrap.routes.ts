  ...
  app.get(legendsMeBootstrapContract.path, async (request, reply) => {
    ...
    const recentAwards = awardSnaps
      ...
      .map((row) => ({
        ...
        id: String(row.id ?? row.awardId ?? ""),
        ...(typeof row.kind === "string" && row.kind ? { kind: row.kind } : {}),
        ...(typeof row.family === "string" && row.family ? { family: row.family } : {}),
        ...(typeof row.dimension === "string" && row.dimension ? { dimension: row.dimension } : {}),
        ...(typeof row.iconContext === "string" && row.iconContext ? { iconContext: row.iconContext } : {}),
        activityKey: row.activityKey == null ? null : String(row.activityKey),
        activityLabel: row.activityLabel == null ? null : String(row.activityLabel),
        locationKey: row.locationKey == null ? null : String(row.locationKey),
        locationLabel: row.locationLabel == null ? null : String(row.locationLabel),
        comboKey: row.comboKey == null ? null : String(row.comboKey),
        rank: row.rank == null ? null : Math.max(1, finiteInt(row.rank, 1)),
        isPermanent: row.isPermanent === true,
        claimedAt: row.claimedAt ?? null,
        updatedAt: row.updatedAt ?? null,
        sourcePostId: row.sourcePostId == null ? null : String(row.sourcePostId),
        ...(row.viewerStatus == null ? {} : { viewerStatus: String(row.viewerStatus) }),
        ...(row.displayPriority == null ? {} : { displayPriority: Math.max(0, finiteInt(row.displayPriority, 0)) }),
    ...
    return success({
      ...
      recentAwards,
      firstLegends: recentAwards.filter((a) => a.family === "first"),
      rankLegends: recentAwards.filter((a) => a.family === "rank"),
      recentEvents,
      defense: {
